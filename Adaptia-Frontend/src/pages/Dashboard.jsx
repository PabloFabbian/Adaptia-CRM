    ...
    return (
        <div className="max-w-7xl mx-auto px-4 pt-3 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-4xl font-light text-gray-900 dark:text-white tracking-tight">
                        {greeting}, <span className="font-bold">{user?.name || 'Profesional'}</span> 👋🏼                    </h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {/* Citas Hoy */}
                <div className="bg-white dark:bg-[#161f31] p-7 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm transition-all hover:shadow-md group">
                    <div className="flex justify-between items-start mb-6">
                        <div className="p-4 rounded-2xl bg-gray-900 dark:bg-[#101828] text-white shadow-lg">
                            <Clock size={24} />
                        </div>
                        <div className="flex flex-col items-end">
                            <span className="text-[10px] font-black text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1 rounded-full uppercase">Al día</span>
                        </div>
                    </div>
                ...
                {/* Pacientes Totales */}
                <div className="bg-white dark:bg-[#161f31] p-7 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm transition-all hover:shadow-md">
                    <div className="flex justify-between items-start mb-6">
                    ...
                    <h3 className="text-4xl font-bold text-gray-900 dark:text-white mt-1">124</h3>
                </div>

                {/* Info Rotativa */}
                <div
                    onClick={rotateInfo}
                    className="bg-white dark:bg-[#161f31] p-7 rounded-[2.5rem] border-2 border-dashed border-gray-100 dark:border-gray-800 shadow-sm hover:border-[#50e3c2] transition-all cursor-pointer group relative overflow-hidden"
                >
                    <div className="flex justify-between items-start mb-6">
            ...
            <section className="space-y-6">
                <div className="flex items-center justify-between px-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">Próximas Citas</h2>
                    </div>
                    <button
                        onClick={() => setIsFocusMode(!isFocusMode)}
                        className={`text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full transition-all ${isFocusMode ? 'bg-[#50e3c2] text-gray-900' : 'bg-gray-100 dark:bg-white/5 text-gray-500'}`}
                    >
                        {isFocusMode ? 'Vista Normal' : 'Modo Enfoque'}
                    </button>
                </div>

                <div className={`bg-white dark:bg-[#161f31] border border-gray-100 dark:border-gray-800 rounded-[3rem] shadow-sm overflow-hidden p-4 transition-all ${isFocusMode ? 'ring-4 ring-[#50e3c2]/10' : ''}`}>
                    {appointments.length > 0 ? (
                        ...
                        <div className="py-24 text-center">
                            ...
                            <h4 className="text-gray-900 dark:text-white font-bold">Todo en orden</h4>
